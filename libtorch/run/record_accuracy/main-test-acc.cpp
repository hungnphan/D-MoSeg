...
int main(int argc, char* argv[]){
    ...
    if (rank_me == 0){
        ...
        for(int i=0;i<rank_n;i++){
            // Create config for the model
            Config config_eval;
            std::pair<std::string,std::string> data_of_me = query_data(i);
            config_eval.scenario_name = data_of_me.first;
            config_eval.sequence_name = data_of_me.second;

            // Initialize custom dataset for foreground training
            auto dataset_eval = CustomDataset(
                0,                              // proc_id
                1,                              // num_proc   
                config_eval.FG_TRAINING_FRAME,  // data_dir
                data_of_me.first,               // scenario_name
                data_of_me.second               // sequence_name
            ).map(torch::data::transforms::Stack<>());

            // Generate a data loader with fixed batch size
            auto data_loader_eval = torch::data::make_data_loader<torch::data::samplers::RandomSampler>(
                dataset_eval,
                data_loader_options
                // batch_size
            );
            ...
            // Start data loop via batches
            for (auto& batch : *data_loader_eval) {
                ...
                // Calculate loss & acc
                loss_eval += loss.item<float>();
                acc_eval += acc.item<float>();
                ...
                if( (is_logging == 1) && ((pass_num) % logging_frequency == 0) )
                    std::cout << "step = " << (pass_num) << "\t"
                            ...
                            << "num_events = " << num_events << "\t"
                            << std::fixed << std::setprecision(7) << "\t"
                            << "Loss = " << loss_eval/pass_num << "\t"
                            << "Acc = " << acc_eval/pass_num << "\n";
        ...
        // Report accuracy
        std::ofstream result_file;
        result_file.open ("benchmark_accuracy.csv", std::ofstream::app);
        result_file << batch_size << "," 
                    << rank_n << "," 
                    << constant << ","
                    << acc_eval << "\n";
